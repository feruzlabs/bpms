...
import com.bpms.connectors.creditconveyer.exceptions.CheckKATMBanFailedException;
...
/** id = eski {@code @Component("CheckKATMBanV8Connector")} */
public final class CheckKATMBanV8Connector implements Connector {

    private final CheckKATMBanV8Service service;

    public CheckKATMBanV8Connector(CheckKATMBanV8Service service) {
        this.service = service;
    }

    @Override
    public String id() {
        return "CheckKATMBanV8Connector";
    }
    ...
    public ConnectorResult execute(ConnectorContext ctx) {
        ...
        String isKatmBanVar = Io.name(in.get("IsKATMBanVarSet"));
        String errorMsgVar = Io.name(in.get("ErrorMsgVarSet"));
        String responseVar = Io.name(in.get("ResponseVarSet"));

        Map<String, Object> out = new HashMap<>();
        try {
            Object response = service.refresh(token);
            Io.put(out, responseVar, response);
            Io.put(out, isSuccessVar, true);
            Io.put(out, isKatmBanVar, false);
        } catch (CheckKATMBanFailedException e) {
            Io.put(out, isSuccessVar, true);
            Io.put(out, isKatmBanVar, true);
        } catch (Exception e) {
    ...
    public ConnectorDescriptor describe() {
        return new ConnectorDescriptor(id(), "v8 check KATM ban",
                List.of(
                        ...
                        new ConnectorInputDesc("IsKATMBanVarSet", false, "string", "KATM ban flag var name"),
