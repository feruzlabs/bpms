...
/**
 * id = eski {@code @Component("CreateRequestAndClientV6Connector")}.
 * Builds {@link CreateNewRequestV6DTO} from SpEL-evaluated {@code ctx.inputs()}
 * (old connector did the same — no CreditConveyorHelper).
 */
public final class CreateRequestAndClientV6Connector implements Connector {

    private final CreateScoringRequestV6Service requestService;

    public CreateRequestAndClientV6Connector(CreateScoringRequestV6Service requestService) {
        this.requestService = requestService;
    }

    @Override
    public String id() {
        return "CreateRequestAndClientV6Connector";
    }
    ...
    public ConnectorResult execute(ConnectorContext ctx) {
        ...
        try {
            CreateNewRequestV6DTO dto = CreateNewRequestV6DTO.builder()
    ...
    public ConnectorDescriptor describe() {
        return new ConnectorDescriptor(id(), "v6 create scoring request and client",
                List.of(
