...
/** id = eski {@code @Component("GetClientInfoV6Connector")} */
public final class GetClientInfoV6Connector implements Connector {

    private final GetScoreLogV6Service service;

    public GetClientInfoV6Connector(GetScoreLogV6Service service) {
        this.service = service;
    }

    @Override
    public String id() {
        return "GetClientInfoV6Connector";
    }
    ...
    public ConnectorResult execute(ConnectorContext ctx) {
        ...
        try {
            ScoringLogV6DTO resp = service.refresh(token);
            String claimId = "";
            try {
    ...
    public ConnectorDescriptor describe() {
        return new ConnectorDescriptor(id(), "v6 get client info",
                List.of(
