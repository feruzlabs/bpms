...
public class RefreshActiveAccountV8Service {
    @Value("${creditconveyer.endpoint}")
    String endpoint;

    private final RefreshService<ActiveAccountResponse, ErrorResponseDTO> refreshService; // Inject this service

    public ActiveAccountResponse refresh(String token) throws ServerErrorGetTokenAuthException, IOException, RefreshServiceErrorException, NotAuthorizedCreateNewRequestException {
        try {
            return refreshService.refresh(endpoint, ConveyorPaths.REFRESH_ACTIVE_ACCOUNT_DATA_V8, token, ActiveAccountResponse.class, ErrorResponseDTO.class);
