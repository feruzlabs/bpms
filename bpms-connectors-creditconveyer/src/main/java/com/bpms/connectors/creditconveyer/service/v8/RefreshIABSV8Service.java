...
public class RefreshIABSV8Service {
    @Value("${creditconveyer.endpoint}")
    String endpoint;

    private final RefreshService<RefreshServiceResponseDTO, ErrorResponseDTO> refreshService; // Inject this service

    public RefreshServiceResponseDTO refresh(String token) throws ServerErrorGetTokenAuthException, IOException, RefreshServiceErrorException, NotAuthorizedCreateNewRequestException {
        try {
            return refreshService.refresh(endpoint, ConveyorPaths.REFRESH_IABS_DATA_V8, token, RefreshServiceResponseDTO.class, ErrorResponseDTO.class);
